  ...
  return (
    <header className="w-full">
      <div className="fixed z-50 flex w-full items-center justify-between p-[0.75rem] mix-blend-difference sm:p-[1rem]">
        {/* logo */}
        <Link href="/" className="caption text-primary-foreground">
          ltrq<span className="text-[#43E4DE]">.</span>
        </Link>
        ...
        <button
          onClick={toggleMenu}
          className={cn(
            'relative flex size-[1rem] cursor-pointer items-center justify-center bg-transparent px-[0.75rem] sm:px-[1rem]',
            'before:absolute before:h-[0.08rem] before:w-[1.25rem] before:bg-primary-foreground before:transition-all before:duration-1000 before:content-[""] before:sm:h-[0.1rem] before:sm:w-[1.5rem]',
            'after:absolute after:h-[0.08rem] after:w-[1.25rem] after:bg-primary-foreground after:transition-all after:duration-1000 after:content-[""] after:sm:h-[0.1rem] after:sm:w-[1.5rem]',
            isMenuOpen
              ? 'before:translate-y-0 before:rotate-45 after:translate-y-0 after:-rotate-45'
              : 'before:translate-y-[-0.25rem] after:translate-y-[0.25rem]',
          )}
        />
      </div>
            ...
            {MENU_ITEMS.map((item, index) => (
              ...
              >
                <Link
                  ...
                  onClick={closeMenu}
                  className={cn(
                    'display-xl relative block text-center text-primary-foreground will-change-transform',
